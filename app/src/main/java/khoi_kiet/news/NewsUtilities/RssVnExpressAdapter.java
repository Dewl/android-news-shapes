...
public class RssVnExpressAdapter implements RssAdapter {
    ...
    public static RssAdapter getInstance() {
        if (instance == null) {
            instance = new RssVnExpressAdapter();
        }
        return instance;
    }
    ...
    public ArrayList<Item> get(String url) throws IOException {
        ...
        for (Element item : items) {	// For each item
            ...
            String img = desRefined.select(IMG).first().attr(SRC);
            // get thumbnail image
